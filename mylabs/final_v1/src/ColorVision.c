...
int flagcolor=0;
float blueobjectx=0.0;
float blueobjecty=0.0;
int bluenumpels=0.0;
float greenobjectx=0.0;
float greenobjecty=0.0;
int greennumpels=0.0;
float pinkobjectx=0.0;
float pinkobjecty=0.0;
int pinknumpels=0.0;
...
void userProcessColorImageFunc_laser(bgr *ptrImage) {
    ...
    if (ptrImage != NULL) {
        ...
        {
            // set specs for blue
            specs_h = 171;       // old:158       //[141:175]
            specs_hrad = 11;     //old:11
            if((specs_h-specs_hrad)<0) // wrap 0->360
            {
                specs_h2=specs_h+256;
            }
            ...
            specs_s = 147;       // old:129    //[101:156]
            specs_srad = 108;     //old:27
            specs_v = 220;    //old:213        //[171:255]
            specs_vrad = 35;    //old:42
            flagcolor=1;
        }
        ...
        //Uncomment if want to look for green and change flagcolor in line above to =2
        //        else if (flagcolor ==2)
        //        {
        //            // set specs for green
        //            specs_h = 105; //[75:135]
        ...
        for (r=5;r<IMAGE_ROWS-75;r++) {
            for(c=4;c<IMAGE_COLUMNS-5;c++) {
                ...
                if ( (abs(sat-specs_s)<=specs_srad)
                    ...
                    if (r == 5) top = 0;  else top = Thres_Image[(r-1)*IMAGE_COLUMNS+c];  // previous row, same column
                    if (c == 4) left = 0; else left = Thres_Image[r*IMAGE_COLUMNS+(c-1)];  // same row, previous column
        ...
        if (new_coordata == 0) {
            if (final_object_stats[largest_object].num_pixels_in_object > 1) {
                ...
                if (flagcolor == 1)
                {
                    blueobjectx=cbar - IMAGE_COLUMNS/2;
                    blueobjecty=rbar - IMAGE_ROWS/2;
                    bluenumpels=final_object_stats[largest_object].num_pixels_in_object;
                }
                else if (flagcolor == 0)
                {
                    pinkobjectx=cbar - IMAGE_COLUMNS/2;
                    pinkobjecty=rbar - IMAGE_ROWS/2;
                    pinknumpels=final_object_stats[largest_object].num_pixels_in_object;
                }

                //				else if (flagcolor == 0)
                //				{
                //				    greenobjectx=cbar - IMAGE_COLUMNS/2;
                //				    greenobjecty=rbar - IMAGE_ROWS/2;
                //				    greennumpels=final_object_stats[largest_object].num_pixels_in_object;
                //				}
