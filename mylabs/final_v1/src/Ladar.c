...
float Ladardata_x[LADAR_MAX_DATA_SIZE];
float Ladardata_y[LADAR_MAX_DATA_SIZE];
...
void LADAR_Get_and_Process_Scan(void)
{
    ...
    {	// upate the left, right center values
        ...
        int j = 0;
        for(j = 0; j<LADAR_MAX_DATA_SIZE; j++){
            Ladardata_x[j] = newLADARdataX[j];
            Ladardata_y[j] = newLADARdataY[j];
        }
    }
