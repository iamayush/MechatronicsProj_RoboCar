...
void assign_penalty(void){
    int r = 0;
    int c = 0;
    for (r = 0; r < mapRowSize; r++){
        for (c = 0; c< mapColSize; c++){
            if (((r > 6) && (r < 9)) && ((c > 2) && (c < 8))){
                penalty[r*mapColSize + c] =  15;
            } else {
                penalty[r*mapColSize + c] = 0;
            }
        }
    }
}
...
void astar(int rowStart, int colStart, int rowEnd, int colEnd)
{
    ...
    while(openSet.numElems > 0 && goalFound != 't')
    {
        ...
        for(cnt = 0; cnt<numNeighbors; cnt++)   //for each found neighbor
        {
            ...
            next.distTravelFromStart = currDist +1 + penalty[next.row*mapColSize + next.col];
            ...
            next.distToGoal = heuristic(minDistNode.row, minDistNode.col, rowEnd, colEnd);
