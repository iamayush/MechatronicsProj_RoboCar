...
int robotdestSize = 12;  // number of positions to use out of robotdest
...
float x_weed_world[5];
float y_weed_world[5];
int pink_tally = 0;
...
void RobotControl(void) {
    ...
    if (settlegyro < SETTLETIME) {
        ...
        //        int Fminladar = LADARdistance[113];
        //        for (i = 111;i<115;i++){
        ...
        //        }
        //
        //        int Rminladar = LADARdistance[54];
        //        for (i = 52;i<58;i++){
        //            if(LADARdistance[i]<Rminladar){
        //                Rminladar = LADARdistance[i];
        //            }
        //        }
        //
        //        int Bminladar = LADARdistance[25];
        //        for (i = 25;i<30;i++){
        //            if(LADARdistance[i]<Bminladar){
        //                Bminladar = LADARdistance[i];
        //            }
        //        }
        //
        //        int BLminladar = LADARdistance[225];
        //        for (i = 220;i<225;i++){
        //            if(LADARdistance[i]<BLminladar){
        //                BLminladar = LADARdistance[i];
        //            }
        //        }
        //
        //        int Lminladar = LADARdistance[200];
        //        for (i = 198;i<202;i++){
        //            if(LADARdistance[i]<Lminladar){
        //                Lminladar = LADARdistance[i];
        //            }
        //        }
        //
        //        int minladar_y = Ladardata_y[113];
        //        for(i = 111; i<116; i++){
        //            if(Ladardata_y[i]<minladar_y){
        //                minladar_y = Ladardata_y[i];
        //            }
        //        }
        //
        //        x_rcoord = cos(ROBOTps.theta)*robotdest[statePos].x + sin(ROBOTps.theta)*robotdest[statePos].y - cos(ROBOTps.theta)*ROBOTps.x - sin(ROBOTps.theta)*ROBOTps.y;
        //        y_rcoord = -sin(ROBOTps.theta)*robotdest[statePos].x + cos(ROBOTps.theta)*robotdest[statePos].y  + sin(ROBOTps.theta)*ROBOTps.x - cos(ROBOTps.theta)*ROBOTps.y;
        //
        //        curr2goal_dist =  sqrt((ROBOTps.x - robotdest[statePos].x )*(ROBOTps.x - robotdest[statePos].x)+ (robotdest[statePos].y - ROBOTps.y)*(robotdest[statePos].y - ROBOTps.y));
        //        dot_prod = (robotdest[statePos].y - ROBOTps.y)*(robotdest[statePos].y - hit_y) + (robotdest[statePos].x - ROBOTps.x)*(robotdest[statePos].x - hit_x);
        //        curr_angle = acos(dot_prod/(curr2goal_dist*hit2goal_dist));
        ...
        if (j == pathLen){
            do_astar = 1;
            if ((statePos < 11) && (astar_weed == 0))
                statePos++;
            else if ((astar_weed == 0) && (statePos >= 11))
                statePos = 11;
            else if (astar_weed == 1)
                astar_weed = 0;
                weednum++;
                weed_centered = 0;
        ...
        //            { statePos = (statePos+1)%robotdestSize; }
        //
        //            if((Fminladar < 400) && (minladar_y<11)){
        //                vref = 0;
        //                turn = 0;
        //
        //
        //
        //                if ((timecount%2) == 0/*x_rcoord > 0*/){
        //                    mode = 3;
        //                    LW_timer = 0;
        //                }
        //                else{
        ...
        //                hit_x = ROBOTps.x;
        //                hit_y = ROBOTps.y;
        //                hit2goal_dist = sqrt((ROBOTps.x - robotdest[statePos].x )*(ROBOTps.x - robotdest[statePos].x)+ (robotdest[statePos].y - ROBOTps.y)*(robotdest[statePos].y - ROBOTps.y));
        //
        //
        //
        //
        ...
        //            RW_timer++;
        //
        //            front_wall_error = 3000 - Fminladar;
        //            right_wall_error = RWdist - Rminladar;
        //
        //            if(fabsf(front_wall_error) > FWdist){
        //                vref = FTvel;
        //                turn = KpFW*front_wall_error;
        //            }
        //            else{
        //                turn = KpRW*right_wall_error;
        //                vref = 1;
        //                if (fabsf(right_wall_error) > BWdist){
        //                    turn = KpBW*(RWdist - Bminladar);
        //                    vref = 0.8;
        //                }
        //            }
        //
        ...
        //            }
        //
        //            if ((curr_angle < 0.18) && (RW_timer > 5000) && (hit2goal_dist >= curr2goal_dist)){
        //                if ( ((x_rcoord > 0) && (Rminladar > 1000)) || ((x_rcoord <= 0) && (Lminladar > 1000)))
        //                    vref = 0;
        //                turn = 0;
        //                mode = 1;
        //            }
        ...
        //
        //            front_wall_error = 3000 - Fminladar;
        //            left_wall_error = LWdist - Lminladar;
        //
        //            if(fabsf(front_wall_error) > FWdist){
        //                vref = FTvel;
        //                turn = -KpFW*front_wall_error;
        //            }
        //            else{
        //                turn = KpLW*left_wall_error;
        //                vref = 1;
        //                if (fabsf(left_wall_error) > BWdist){
        //                    turn = KpBLW*(BWdist - BLminladar);
        //                    vref = 0.8;
        //                }
        //            }
        //
        ...
        //            }
        //
        //            if ((curr_angle < 0.18) && (LW_timer > 5000) && (hit2goal_dist >= curr2goal_dist)){
        //                if ( ((x_rcoord > 0) && (Rminladar > 1000)) || ((x_rcoord <= 0) && (Lminladar > 1000)))
        //                    vref = 0;
        //                turn = 0;
        //                mode = 1;
        ...
        //        if (numpels_b > 50)
        //        {
        //            color_vision_error = 0 - objectx_b;
        //            //Distance from center of robot to pink weed
        //            color_distance = (0.000425195*pow(objecty_b,3))+(0.074152963*pow(objecty_b,2))+(4.686246680*objecty_b)+127.692244805;
        //
        //        }
        //
        //        else if (numpels_p > 50)
        //        {
        //color_vision_error = 0 - objectx_p;
        //Distance from center of robot to pink weed
        ...
        if((pink_tally > 20) && (weed_centered == 0)){
            color_distance = ((0.000425195*pow(objecty_p,3))+(0.074152963*pow(objecty_p,2))+(4.686246680*objecty_p)+127.692244805)/12.0;
            color_vision_error = 0 - objectx_p;

            if(fabs(objectx_p) > 10){
                vref = 0;
                turn = kp_vision*color_vision_error;
            }
            else{
                weed_centered = 1;
                pink_tally = 0;
                do_astar = 1;
                x_weed_world[weednum] = color_distance*sin(ROBOTps.theta) + ROBOTps.x;
                y_weed_world[weednum] = color_distance*cos(ROBOTps.theta) + ROBOTps.y;
            }
        ...
        //        if ((color_distance) < 22.0) && (numpels_p > 150))
        //        {
        //
        //            //            vref = 0.0;
        //            //            turn = 0.0;
        //            exterminate_timer++;
        //            if (exterminate_timer == 10)
        //            {
        //                vref = 0.0;
        //                turn = 0.0;
        //            }
        ...
        if ((timecount%200)==0) {
            ...
            case 4:
                LCDPrintfLine(1,"Distance_p %.2f",color_distance);
                LCDPrintfLine(2,"B %d P %d",numpels_b, numpels_p);
                break;

            case 5:
                LCDPrintfLine(1,"xW %.1f yW %.1f",x_weed_world[weednum],y_weed_world[weednum]);
                LCDPrintfLine(2,"%c P %d",astar_weed, numpels_p);
                break;
