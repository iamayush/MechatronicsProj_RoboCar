...
void RobotControl(void) {
    ...
    if (settlegyro < SETTLETIME) {
        ...
        case 1:
            ...
            for (x=111; x <=115; x++)
            {
                if (LADARdistance[x] < minimumfront)
                {
                    minimumfront = LADARdistance[x];
                }

            }
            if(minimumfront <750) { // if obstacle in front, stop
                vref = 0.0;
                mode = 2;
            }
            break;
        ...
        case 2:
            ...
            minimumright = 4095;
            int xright=52;
            for (xright=52; xright <=58; xright++)
            {
                if (LADARdistance[xright] < minimumright)
                {
                    minimumright = LADARdistance[xright];
                }
            }

            //        if (LADARdistance[54] < minimumright)
            //        {
            //            minimumrightfinal = LADARdistance[54];
            //        }
            //        else
            //        {
            //            minimumrightfinal=minimumright;
            //        }
            ...
            front_wall_error = 3000 - minimumfront;

            // calculate error between ref_right_wall and right wall measurement
            right_wall_error = ref_right_wall - minimumright;//minimumrightfinal;

            if (fabsf(front_wall_error) > 99999/* front_error_threshold*/){
            ...
            if (turn < (-1.0*(turn_command_saturation)))
            {
                turn = -turn_command_saturation;
            }
        ...
        if ((timecount%200)==0) {
            ...
            case 1:
                ...
                break;
            case 2:
                LCDPrintfLine(1, "FWerr %d",front_wall_error);
                LCDPrintfLine(2, "RWerr %d",right_wall_error);
                break;

            case 3:
                LCDPrintfLine(1, "FWerr %d",front_wall_error);
                LCDPrintfLine(2, "mode %c",mode);
                break;
            }
