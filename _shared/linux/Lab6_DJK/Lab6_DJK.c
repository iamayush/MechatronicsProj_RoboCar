...
int main (int argc, char **argv)
{
	...
	float DVel = 0.0;
	float turn = 0.0;
	float side_distance = 400.0;
	float front_threshold=2500.0;
	float prop_distance =.002;
	float prop_turn=.002;
	float front_velocity =0.4;
	float saturation = 15.0;
	...
	// Set each DSPFloats item to its initial value
	DSPFloats[0] = DVel;
	DSPFloats[1] = turn;
	DSPFloats[2] = side_distance;
	DSPFloats[3] = front_threshold;
	DSPFloats[4] = prop_distance;
	DSPFloats[5] = prop_turn;
	DSPFloats[6] = front_velocity;
	DSPFloats[7] = saturation;
	DSPFloats[8] = 0.0;
	DSPFloats[9] = 0.0;
	DSPFloats[10] = 0.0;
	DSPFloats[11] = 0.0;
	...
	while (!inputdone) {
		...
		printf("w - Distance from side wall\n");
		printf("r - Distance from front wall\n");
		printf("k - Prop gain for wall\n");
		printf("t - Prop turning for wall\n");
		printf("b - front turn vel\n");
		printf("d - turn saturation\n");
		printf("l - lists all parameters\n");
		mychar = (char) mygetch();
		...
		//ref right wall
		case 'w':
		printf("Enter Distance to stay left/right of wall\n");
			...
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
					side_distance = tempfloat;
					printf("Side Distance = %.3f\n",side_distance);
				}  else {
					printf("Error: Non numerical value typed\n");
				}
			} else {
				printf("Error: Side Distance not changed\n");
			}
			...
			break;
		case 'r':
		printf("Enter Distance to stay from front wall\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
					front_threshold = tempfloat;
					printf("Front threshold = %.3f\n",front_threshold);
				}  else {
					printf("Error: Non numerical value typed\n");
				}
			} else {
				printf("Error: Front Threshold not changed\n");
			}
			// send new Vref value to DSP
			DSPFloats[3] = front_threshold;
			writefloats_sharedmem_5times();  // Our hack way of "hopefully" making sure our data is not sitting in cache but in the actual shared memory
			break;
		case 'k':
		printf("Proportional Distance\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
					prop_distance = tempfloat;
					printf("Prop Distance = %.3f\n",prop_distance);
				}  else {
					printf("Error: Non numerical value typed\n");
				}
			} else {
				printf("Error: Prop Distance not changed\n");
			}
			// send new Vref value to DSP
			DSPFloats[4] = prop_distance;
			writefloats_sharedmem_5times();  // Our hack way of "hopefully" making sure our data is not sitting in cache but in the actual shared memory
		...
		printf("Proportional Turn\n");
			...
			if (buffer[0] != '\0') {
				...
				printf("Error: Prop Turn not changed\n");
			}
			// send new Vref value to DSP
			DSPFloats[5] = prop_turn;
			writefloats_sharedmem_5times();  // Our hack way of "hopefully" making sure our data is not sitting in cache but in the actual shared memory
		...
		case 'b':
		printf("Front Turn Velocity\n");
			fgets(buffer,190,stdin); 
			buffer[strlen(buffer)-1] = '\0';  // get ride of '\n' in returned string
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
					front_velocity = tempfloat;
					printf("Front Turn Velocity = %.3f\n",front_velocity);
				}  else {
					printf("Error: Non numerical value typed\n");
				}
			} else {
				printf("Error: Front Velocity not changed\n");
			}
			// send new Vref value to DSP
			DSPFloats[6] = front_velocity;
			writefloats_sharedmem_5times();  // Our hack way of "hopefully" making sure our data is not sitting in cache but in the actual shared memory
		...
		printf("Saturation\n");
			...
			if (buffer[0] != '\0') {
				if (sscanf(buffer,"%f",&tempfloat) != 0) {  // check that it was a number entered
					saturation = tempfloat;
					printf("Saturation = %.3f\n",saturation);
				}  else {
					printf("Error: Non numerical value typed\n");
				}
			} else {
				printf("Error: Saturation not changed\n");
			}
			// send new Vref value to DSP
			DSPFloats[7] = saturation;
			writefloats_sharedmem_5times();  // Our hack way of "hopefully" making sure our data is not sitting in cache but in the actual shared memory
		...
		printf("VrEF %.1f \n\r",DSPFloats[0]);
		printf("Turn %.1f \n\r",DSPFloats[1]);
		printf("Side Distance %.1f \n\r",DSPFloats[2]);
		printf("Front Threshold %.1f \n\r",DSPFloats[3]);
		printf("Proportional Gain Distance %.4f\n\r",DSPFloats[4]);
		printf("Proportional Gain Turn %.4f \n\r",DSPFloats[5]);
		printf("Front Turn Velocity %.1f \n\r",DSPFloats[6]);
